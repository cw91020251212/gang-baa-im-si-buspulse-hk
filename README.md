...
- The detail summary shows the official full-route fare from the committed fare index. KMB routes also load the operator's official timetable endpoint, so the direction-aware summary shows first bus, last bus, and the current frequency window when available; the expandable service section links back to the official route source.
...
- In the detail timeline, keyboard users can press **↑／↓** on a station row to move to the previous or next stop; the selected row, live dock, and three ETA cards stay synchronized.
- The detail status separates the local fetch time from the provider's `data_timestamp` when the official response supplies one, so users can judge freshness without mistaking the request time for vehicle-location time.
...
The detail summary also applies the project-approved **ETA 時間斷層法** to the first live ETA at each stop. A backward jump of more than one minute starts another possible vehicle wave, and the result is shown as **ETA 推算行駛中：N 部**. This is an ETA-based estimate under the open-data constraints, not a GPS or vehicle-ID count; the expandable service section explains the method.
