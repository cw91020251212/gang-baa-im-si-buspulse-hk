...
On Android, open the site in Chrome, allow notifications, use **「加入主畫面」**, open the installed app once, add a route, and enable its bell. Background Push is kept on and synchronized automatically; **「真正關閉」** is the manual switch for stopping it. Android system notification permission and battery optimisation settings still control delivery. GitHub Pages alone cannot run a server-side poller, and this repository intentionally does not contain VAPID private keys.
